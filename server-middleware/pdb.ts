...
export async function anchorPOI(poi: POI) {
  ...
  const client = anchor.connect(
    anchor.withCredentials(process.env.PROVENDB_SDK_KEY || 'sdk_key'),
    anchor.withInsecure(
      (process.env.PROVENDB_SDK_INSECURE === 'true' && true) || false
    ),
    anchor.withAddress(
      process.env.PROVENDB_SDK_ENDPOINT || 'anchor.proofable.io:443'
    )
  )
  ...
  try {
    const proof = await client.submitProof(
      tree.getRoot(),
      anchor.submitProofWithAnchorType(
        process.env.PROVENDB_SDK_ANCHOR || anchor.Anchor.Type.HEDERA_MAINNET
      ), // Optional. Add your anchor type.
      anchor.submitProofWithAwaitConfirmed(
        (process.env.PROVENDB_SDK_AWAIT === 'true' && true) || false
      ),
      anchor.submitProofWithSkipBatching(
        (process.env.PROVENDB_SDK_SKIP === 'true' && true) || true
      )
    ) // Optional. Resolve the promise only when the proof is confirmed.

    LOGGER.debug({
      message: 'Result of Submitting Proof',
      code: poi.code,
      proof,
      tree,
    })
...
export async function checkProof(proof) {
  ...
  try {
    ...
    LOGGER.error({ message: 'Failed to fetch proof', id: proof.id, e })
    return false
  }
}
